...
export function GuestNameScreen({ onContinue, onBack }: GuestNameScreenProps) {
  ...
  return (
    <Box sx={styles.screen}>
      ...
      <Box sx={styles.card}>
        ...
        <Typography sx={styles.subtitle}>Just your name. Nothing else.</Typography>

        <Box component="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter your name" sx={styles.input as any} />

        <Box sx={styles.buttonRow}>
          <Box component="button" onClick={() => onContinue(name.trim())} sx={{ ...styles.button, ...styles.primaryButton }}>Continue</Box>
