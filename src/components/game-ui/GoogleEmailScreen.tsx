...
interface GoogleEmailScreenProps {
  onContinue: (email: string) => void;
  onBack: () => void;
  email: string;
}

export function GoogleEmailScreen({ onContinue, onBack, email: initialEmail }: GoogleEmailScreenProps) {
  ...
  return (
    <Box sx={styles.screen}>
      ...
      <Box sx={styles.card}>
        <Typography sx={styles.title}>Google Login</Typography>
        <Typography sx={styles.subtitle}>Enter your Gmail to restore your progress later.</Typography>

        <Box component="input" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@gmail.com" sx={styles.input as any} />

        <Box sx={styles.buttonRow}>
          <Box component="button" onClick={() => onContinue(email.trim().toLowerCase())} sx={{ ...styles.button, ...styles.primaryButton }}>Continue with Google</Box>
